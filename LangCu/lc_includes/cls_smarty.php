...
* ============================================================================
* Template System Of LangCu
* MODIFY FROM ECSHOP SMARTY TEMPLATE
* ============================================================================
...
class cls_smarty
{
    ...
    var $_var           = array();
    var $_lcHash        = 'xuzhipingtesttesttesttesttesttes';
    var $_foreach       = array();
    var $_current_file  = '';
    var $_expires       = 0;
    var $_errorlevel    = 0;
     ...
     * assign all variable
     */
    function assign($assign_name, $value = '')
    {
        if (is_array($assign_name))
        {
            foreach ($assign_name AS $key => $val)
            {
                if ($key != '')
                {
                    $this->_var[$key] = $val;
    ...
    function display($filename, $cache_id = '')
    {
        ...
        if (strpos($out, $this->_lcHash) !== false)
        {
            $k = explode($this->_lcHash, $out);
        ...
        else
        {
            ...
            else
            {
                ...
                if ($cache_id)
                {
                    ...
                    $hash_dir = $this->cacheDir . '/' . substr(md5($cachename), 0, 1);
                    if (!is_dir($hash_dir))
                    {
                        mkdir($hash_dir);
                    }
    ...
    /**
     * judge if the template is being cached
     */
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $out .= 'echo $this->_lcHash . $k[\'name\'] . \'|\' . serialize($k) . $this->_lcHash;' . "\n?>";
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        $tmp_dir   = $GLOBALS['smartDir'] . '/'; // 模板所在路径
